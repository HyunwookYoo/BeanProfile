...
**알려진 한계(실측):** 브랜드가 가장 큰 텍스트인 커피 **봉투**(bag)에서는 이 타이포 휴리스틱이 뒤집힌다 — 브랜드가 `제품명`으로, 태그라인이 `로스터리`로 채워진다. 승인된 휴리스틱(최대 폰트 줄=제목) 자체에 내재한 한계이며, 사용자가 저장 전 폼을 검토하므로 치명적이지 않다. 이 마일스톤의 실기기 검증은 **정보 카드(info card)** 레이아웃만 다루며, 봉투 레이아웃은 검증 범위 밖이다.
