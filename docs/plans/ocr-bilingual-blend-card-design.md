...
### 구현 중 드러난 후속 과제 (2026-08-05, 전체-브랜치 리뷰)

- **성분 `region`에 "모르면 null" 규칙이 없다.** 이 카드에는 지역 라벨이 아예 없는데도 `_unlabeledSpatialFields`가 잔여 텍스트를 지역으로 채운다 — 측정값 `bio control 70940%`(Thailand) · `GI -`(Ethiopia) · `Papayo`(Colombia). 설계가 `ratioPercent`에 대해 명시적으로 거부한 것과 같은 실패인데 `region`에는 그 규칙이 없다. **부수 효과**: 쓰레기 지역이 `filledFieldCount`를 부풀리고, 그 값이 `compareOcrCandidates`의 타이브레이커라 2패스 스캔의 ORIGINAL/ENHANCED 선택을 뒤집을 수 있다. 시작 가설 — `_unlabeledRegion`이 `%`를 남기거나 숫자가 과반인 잔여를 거부하면 앞의 둘은 null이 되고 `Papayo`·`후일라`는 살아남는다. 현재 값은 테스트에 **의도적으로 고정**해 뒀으니, 이 규칙을 넣으면 테스트가 시끄럽게 깨지면서 의식적 결정을 강제한다.
- **한 줄 라벨 + 여러 행 컵노트는 과소수집된다(잠재).** `_cupNoteBlockValues`의 `block.length < 2` 가드는 값 열이 아니라 **라벨**이 2줄인지로 판단한다. 한 줄짜리 `컵노트` 라벨 아래 노트가 행마다 하나씩 놓인 카드는 첫 줄만 잡힌다. 실카드 사례는 아직 없고, 과소수집은 폼에서 칩으로 복구되므로 과수집보다 낫다는 판단으로 수용했다.
- **어휘 항목은 테스트로 고정되지 않는다.** `_otherLabelTokens`에서 `블렌딩`·`블렌딩정보`·`blending info`를 전부 빼도 스위트가 초록이다. `로스터기`만 F2 테스트가 잡는다.

### 방법론 교훈 — 이 파서에는 뮤테이션 확인이 필요하다

태스크 리뷰 4개를 전부 통과한 테스트 **3개가 무력**했다. 가드를 지워도 스위트가 초록이었고, 전체-브랜치 리뷰가 뮤테이션 프로브(가드 삭제 → 재실행)로 잡았다. 원인은 공통적이다 — 나중에 추가된 더 넓은 가드가 먼저 발동해서, 앞서 설계한 판별 테스트가 조용히 판별력을 잃었다. 특히 §4.1의 "어휘 추가는 장식이 아니라 이 규칙의 전제다"를 증명한다던 테스트가 그걸 증명하지 못하고 있었다.

이 프로젝트에서 같은 일이 두 번째다(디개싱 마이그레이션 테스트가 `onUpgrade`를 통째로 지워도 통과했다). **이 파서의 새 가드에는 "지워보고 빨개지는지" 확인을 표준 절차로 둔다.**
